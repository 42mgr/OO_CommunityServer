...
namespace ASC.Mail.Runtime
{
    ...
    public class RuntimeCrmMonitor
    {
            ...
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                while (reader.Read())
                {
                    emails.Add(new EmailInfo {
                        ...
                        Tenant = reader.GetInt32(1),
                        From = reader.IsDBNull(2) ? "" : reader.GetString(2),
                        To = reader.IsDBNull(3) ? "" : reader.GetString(3),
                        Cc = reader.IsDBNull(4) ? "" : reader.GetString(4),
        ...
        private static bool ProcessEmailForCrm(MySqlConnection conn, EmailInfo email)
        {
        ...
        private static void CreateCrmRelationshipEvent(MySqlConnection conn, EmailInfo email, ContactInfo contact, string matchingEmail)
        {
            var insertQuery = @"
        ...
        private static void CreateNoMatchEvent(MySqlConnection conn, EmailInfo email)
        {
            var insertQuery = @"
                INSERT INTO crm_relationship_event 
                (contact_id, entity_type, entity_id, content, create_on, create_by, tenant_id, category_id, have_files)
